...
var idParam = params.get('job-id');
console.log(idParam);
var seeDetail = function () { return __awaiter(_this, void 0, void 0, function () {
    var job, articleJob, contCard, cardJob, titleJob, txtTitle, description, txtDescription, filterLocation, txtLocation, filterSeniority, txtSeniority, filterCategory, txtCategory, contBtn, btnEdit, txtBtnEdit, btnDelete, txtBtnDelete;
    var _this = this;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4 /*yield*/, getJob(idParam)];
            case 1:
                job = _a.sent();
                console.log(job);
                detailsContainer.innerHTML = '';
                articleJob = document.createElement('article');
                articleJob.classList.add('col');
                contCard = document.createElement('div');
                contCard.classList.add('card');
                cardJob = document.createElement('div');
                cardJob.classList.add('card-body');
                titleJob = document.createElement('h5');
                titleJob.classList.add('card-title');
                txtTitle = document.createTextNode(job.name);
                description = document.createElement('p');
                description.classList.add('card-text');
                txtDescription = document.createTextNode(job.description);
                filterLocation = document.createElement('span');
                filterLocation.className = 'badge text-bg-info m-1';
                txtLocation = document.createTextNode(job.location);
                filterSeniority = document.createElement('span');
                filterSeniority.className = 'badge text-bg-info m-1';
                txtSeniority = document.createTextNode(job.seniority);
                filterCategory = document.createElement('span');
                filterCategory.className = 'badge text-bg-info m-1';
                txtCategory = document.createTextNode(job.category);
                ...
                txtBtnEdit = document.createTextNode('Edit');
                ...
                txtBtnDelete = document.createTextNode('Delete');
                ...
                btnDelete.addEventListener('click', function () { return __awaiter(_this, void 0, void 0, function () {
                    return __generator(this, function (_a) {
                        switch (_a.label) {
                            case 0: return [4 /*yield*/, deleteJob(idParam)];
                            case 1:
                                _a.sent();
